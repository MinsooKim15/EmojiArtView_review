...
class ImageFetcher
{
    ...
    //Example: let fetcher = ImageFetcher(){// the code to execute when fetch is done}
    ...
    // The backup can be set at anytime (i.e. before, during or after the fetch)
    // If the fetch fails and a backup image is never set, the handler will never be called.
    // Thus it would sort of be a strange use of this calss to not set a backup image
    ...
    // otherwise the result of the fetch will be discarded and the hanlder never called.
    // In other words, keeping a strong pointer to your instance says's "I'm still interested in its result"
...
extension URL{
    var imageURL: URL {
        if let url = UIImage.urlToStoreLocallyAsJPEG(named: self.path){
            //this was created using UIImage.sotreLocallyAsJPEG
            return url
        }else{
